...
{
    /// <summary>
    /// General purpose signal processing service.
    ...
    public static class SignalService
    {
        ...
        /// </summary>
        static List<Delegate>[] _reactMethods;
...
#region AddSignal
        ...
        /// </summary>
        public static void AddSignal(int id) => _signals.Enqueue(id);
        ...
        internal static void BindSignals(MethodInfo[] signals)
        {
            ...
            for (int i = 0; i < signals.Length; i++)
            {
                ...
                {
                    _signalQueueLookup[i] = (queueIndex++, new Type[] { });
        ...
        {
            const BindingFlags BindingFlags = BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Instance;
            Type type = instance.GetType();
            MethodInfo[] methods = type.GetMethods(BindingFlags);

            BindReactiveMethods(methods, type.Name);
        }
        ...
        internal static void AddReactiveService(Type system)
        {
            ...
            BindReactiveMethods(system.GetMethods(BindingFlags), system.Name);
        }

        // ReSharper disable once ParameterTypeCanBeEnumerable.Local
        static void BindReactiveMethods(IReadOnlyList<MethodInfo> methods, string instanceName)
        {
            foreach (MethodInfo method in methods)
            {
                ...
                // todo: internally Assert throws an exception therefore in case of multiple mismatches the user
                // todo: will learn it one by one even tho all information could be given in one go
                Assert.IsFalse(id == int.MinValue,
                    $"There is a [React] method called On{name} in {instanceName} but no matching signal in ISignal.cs");
