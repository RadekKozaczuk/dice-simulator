...
{
    static readonly int Length = "        public static void () => SignalDispatch.Intercept(\"\", );".Length;

#if DEBUG
    internal static string Code(IEnumerable<SyntaxTree> syntaxTrees, out string prCode, out string DEBUGINFO)
    {
        DEBUGINFO = "";
#else
    internal static string Code(IEnumerable<SyntaxTree> syntaxTrees, out string prCode)
    {
        ...
        int signalId = 0; // and count at the same time
            ...
            for (int i = 0; i < members.Count; i++)
            {
                ...
                else
                {
                    for (int j = 0; j < parameters.Count ; j++)
                    {
                        ...
                        // in case of qualifiedname like SomeClass.SomeEnum
                        else if (parameter.Type is QualifiedNameSyntax qns)
                            typeName = ParseQualifiedName(qns);
                        // other cases, this might not work everytime but can prevent some bugs
                        else
                            typeName = parameters[j].Type!.ToString() + @"/*this type is unexpected in source generator please fix*/";
        ...
        static string GetInterceptString(StringBuilder paramNamesSb, string methodName, ref int signalId) =>
            $"SignalDispatch.Intercept({signalId++}{(paramNamesSb.Length > 0 ? $", {paramNamesSb}" : "")});";
    }
    ...
    static string ParseQualifiedName(QualifiedNameSyntax qns)
    {
        return $"{qns.Left.ToString()}.{qns.Right.ToString()}";
    }
